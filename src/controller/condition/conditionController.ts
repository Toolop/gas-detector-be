...
export default function conditionController(
    ...
    const addNewConditionController = (req: any, res: any, next: any) => {
        try {
            const { upperDanger, upperWarning, lowerDanger, lowerWarning, sensorId } = req.body;
            addConditionUseCase(
                parseInt(upperDanger), parseInt(upperWarning), parseInt(lowerDanger), parseInt(lowerWarning), parseInt(sensorId), dbRepository
            )
                .then((condition: any) => {
                    res.status(201);
                    res.json(condition);
